...
void cpu::op_lb() {
	...
	auto data = read_data(bus::bus_width_t::BUS_WIDTH_BYTE, address);
...
void cpu::op_lh() {
	...
	if (address & 1) {
		...
		auto data = read_data(bus::bus_width_t::BUS_WIDTH_HALF, address);
...
void cpu::op_lwl() {
	auto address = rs() + decoder::iconst();
	auto data = read_data(bus::bus_width_t::BUS_WIDTH_WORD, address & ~3);
...
void cpu::op_sb() {
	...
	write_data(bus::bus_width_t::BUS_WIDTH_BYTE, address, data);
}
...
void cpu::op_sh() {
	...
	if (address & 1) {
		...
		write_data(bus::bus_width_t::BUS_WIDTH_HALF, address, data);
	}
}
...
void cpu::op_sw() {
	...
	if (address & 3) {
		...
		write_data(bus::bus_width_t::BUS_WIDTH_WORD, address, data);
	}
}
...
void cpu::op_swl() {
	...
	write_data(bus::bus_width_t::BUS_WIDTH_WORD, address & ~3, data);
}
